function initSubMenuDisplay() { 
  ...
  Page({
    ...
    gdAdd: function(e) { //新增工单
      wx.navigateTo({
        url: '../../../pagesC/pages/baoxiuWB_add/baoxiuWB_add'
      })
    },
    td: function(e) { //工单详情
      var that = this;
      orderNo = e.currentTarget.dataset.key;
      that.setData( {
        ifName: true,    //显示弹出框
      }); 
    },
    ...
    bindFormSubmit: function (e){
      var tdyy = e.detail.value.stuRecord;
      let that = this;
      that.setData({
        ifName: false,
      });
      that.tdyy_update(userid,orderNo,"1006",tdyy);
    },
    jd: function(e) { //已完成
    ...
    bindMenu1:function(){  //工单详情
      wx.navigateTo({
        url: '../../../pagesC/pages/workOrderWB_info/workOrderWB_info?orderNo='+orderNo
      }),
      this.setData({
        actionSheetHidden:!this.data.actionSheetHidden
      })
    ...
    bindMenu2:function(){  //编辑工单
      ...
      if(LX == '1002'){  //报修工单
        url = '../../../pagesC/pages/baoxiuWB_update/baoxiuWB_update?orderNo='+orderNo;
    ...
    tdyy_update: function(userid,orderNo,djState,tdyy) { //退单
      var that = this;
      var _data = {ac: 'tdyy_update',"userid":userid,"orderNo":orderNo,"djState":djState,"tdyy":tdyy};
      wx.request({
        url: apiUrl,  //api地址
        data: _data,
        header: {'Content-Type': 'application/json'},
