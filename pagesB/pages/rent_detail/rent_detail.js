...
Page({
  ...
  ysk_list:function (fid) { //获取已收款
    let _this = this;
    var _data = {ac: 'ysk_list',"fid":fid};
    wx.request({
      url: apiUrl,  //api地址
      data: _data,
      header: {'Content-Type': 'application/json'},
  ...
  tapSK: function (e) {  //收款
    ...
    wx.navigateTo({
      url: '../../../pagesB/pages/sk_add/sk_add?fid='+id
    })
  },
  tapZRF: function (e) {  //找人付
    let id = e.currentTarget.dataset.key;
    wx.navigateTo({
      url: '../../../pagesB/pages/zrf_info/zrf_info?fid='+id
    })
  },
  tapMore: function (e) {  //显示更多
    ...
    if(jq=="1"){
      czlist.push(
        {bindtap:'Menu1',txt:'编辑账单'},
        {bindtap:'Menu2',txt:'账单改期'},
        {bindtap:'Menu3',txt:'作废账单'},
      )
    }
    else if (jq=="2"){
      czlist.push(
  ...
  onShareAppMessage: function() {
    return {
      title: '账单明细',
      path: '/pages/jztx_info/jztx_info?fid='+fid,
      success: function (res) {
