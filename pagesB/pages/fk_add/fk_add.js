...
Page({
  ...
  onLoad: function (options) {
    ...
    this.get_skmx(fid);  //获取付款明细
    this.get_zffs();  //获取支付方式
    this.get_kssj();
    this.syzd_info(fid);  //获取账单详情
  },
  get_skmx:function (fid) { //获取付款明细
    let _this = this;
  ...
  bindDateChange: function(e) {  //付款日期
    this.setData({
  ...
  formSubmit: function (e){  //保存数据
    ...
    var _data = {ac: 'fkd_add',"contractNo":hth,"userid":userid,"ysje":ysje,"SKdate":SKdate,"zffs":zffs,"htid":htid,"zfje":zfje,"remark":remark};
    wx.request({
      url: apiUrl,  //api地址
      data: _data,
      header: {'Content-Type': 'application/json'},
