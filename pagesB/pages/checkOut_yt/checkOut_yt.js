...
Page({
  ...
  onLoad: function (options) {
    ...
    this.checkOut_qtfk(tzNo);  //获取应退金额
  },
  ...
  checkOut_qtfk:function (tzNo) { //获取应退金额
    let _this = this;
    var _data = {ac: 'checkOut_qtfk',"tzNo":tzNo};
    wx.request({
      url: apiUrl,  //api地址
      data: _data,
      header: {'Content-Type': 'application/json'},
          ...
          if(units.length > 0){
            ...
            for (var i = 0; i < units.length; i++) {
              unit.push({
                "index":i,
                "lsh":units[i].id4,
                "je":units[i].receive_d4,
              });
              je.push(   
                units[i].receive_d4
              );
              fylx.push(  
                _this.get_indexYW(fylxQJ,units[i].costType_d4), 
              );
            }
            setTimeout(()=>{
              _this.setData({
  ...
  delmore(e) {
   var that = this;
   let id4 = e.currentTarget.dataset.key;
   const {inputList} = this.data   //简写变量书写
   const {dataset: {index}} = e.currentTarget
  ...
  ytje_del: function (id4){  //删除应付金额
    var _data = {ac: 'ytje_del',"id4":id4};
    wx.request({
      url: apiUrl,  //api地址
      data: _data,
      header: {'Content-Type': 'application/json'},
 ...
 get_indexYW:function(arrayName,code){
  ...
  formSubmit: function (e){  //保存数据
    ...
    var _data = {ac: 'ytje_save',"tzNo":tzNo,"lshList":mx_lsh,"fylxList":mx_fylx,"jeList":mx_je};
    wx.request({
      url: apiUrl,  //api地址
      data: _data,
      header: {'Content-Type': 'application/json'},
