...
function initSubMenuDisplay() { 
  ...
  Page({
    ...
    onLoad: function (options) {  //生命周期函数--监听页面加载
      ...
      this.get_rzrList(userid,sq,czlx,search,SdDJ,EdDJ); //入住记录
    },
    ...
    get_rzrList:function (userid,sq,czlx,search,Sd,Ed) { //合同列表
      let _this = this;
      fyQJ = [];  //初始化房源数组
      _this.setData({
        servicelist:[]
      ...
      var _data = {ac: 'get_rzrList',"userid":userid,"sq":sq,"czlx":czlx,"search":search,"Sd":Sd,"Ed":Ed};
      wx.request({
        url: apiUrl,  //api地址
        data: _data,
        header: {'Content-Type': 'application/json'},
        ...
        success(res) {
            ...
            const newlist = [];
            var units = res.data.rows;
            var roomTotal = units.length;
            if(roomTotal > 0){
              fyQJ = units;
              page_total = Math.ceil(roomTotal/mysl);  //总页数
              var qty = 0;
              if(roomTotal > page*mysl){
                qty = page*mysl;
              }
              else{
                qty = roomTotal;
              ...
              for (var i = (page-1)*mysl; i < qty; i++) {
                ...
                newlist.push({
                  ...
                  "rzzq":ks+'至'+js,
                  "sf":units[i].rlx,
                  "ts":(units[i].zz=='离开') ? '已离开' : '剩' +units[i].ts+'天',
                })
              }                    
            }
            setTimeout(()=>{
    ...
    submitSearch:function(){  //提交搜索
      ...
      this.get_rzrList(userid,sq,czlx,search,SdDJ,EdDJ);
    },
